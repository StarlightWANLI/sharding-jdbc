...
import com.wanli.shardingjdbc.entity.Address;
...
public class OrderServiceImpl implements ExampleService {
    ...
    private void initAddressTable() throws SQLException {
        mybatisAddressRepository.createTableIfNotExists();
        mybatisAddressRepository.truncateTable();
        for (int i = 1; i <= 10; i++) {
            Address entity = new Address();
            entity.setAddressId((long) i);
            entity.setAddressName("address_" + String.valueOf(i));
            mybatisAddressRepository.insert(entity);
        }
    }
    ...
    public void printData() throws SQLException {
        ...
        for (Object each : mybatisOrderRepository.selectAll()) {
            System.out.println(each);
        }
        System.out.println("---------------------------- Print OrderItem Data -------------------");
        for (Object each : mybatisOrderItemRepository.selectAll()) {
            System.out.println(each);
        }
    }
}
